...
/// NavigationController 重写了代理属性，可以通过此属性设置值，但是此属性返回值始终是内部对象。
...
/// NavigationController 转场过程中的阴影视图。
SWIFT_CLASS("_TtC5XZKit39NavigationControllerAnimationShadowView")
@interface NavigationControllerAnimationShadowView : UIView
- (nonnull instancetype)initWithFrame:(CGRect)frame OBJC_DESIGNATED_INITIALIZER;
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)aDecoder OBJC_DESIGNATED_INITIALIZER;
@end
...
/// 显示占位图的视图。
/// note:
/// 占位图视图会观察当前视图的属性 image、alpha、contentMode、backgroundColor ，并实时同步。
